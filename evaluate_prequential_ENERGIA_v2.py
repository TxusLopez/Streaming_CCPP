...
class EvaluatePrequential_ENERGIA_v2(StreamEvaluator):
...
#                    preds = [[] for _ in range(self.n_models)]
                    for i in range(self.n_models):
                        try:
                            # Testing time
...
#                    self.predicciones.append(preds)
                    self.predicciones.append(prediction)
                    self.verdades.append(y)
...
#        self.evaluation_summary()
        ...
        return self.model,self.predicciones,self.verdades
